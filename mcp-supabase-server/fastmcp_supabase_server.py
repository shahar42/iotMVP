...
async def delete_record(table_name: str, where_clause: str) -> str:
    ...
    try:
        ...
        # Basic safety check for dangerous WHERE clauses
        where_upper = where_clause.upper().strip()
        if where_upper in ['1=1', 'TRUE', '1', 'true']:
            return "Error: Dangerous WHERE clause detected. Please use specific conditions."
        ...
        return f"Error deleting from {table_name}: {str(e)}"
